...
## CURL examples
### Register
```curl -X POST http://x.x.x.x:4000/register -H "Content-Type: application/json" -d '{"username":"your_username", "password":"your_password"}'```

### Login
```curl -X POST http://x.x.x.x:4000/login -H "Content-Type: application/json" -d '{"username":"your_username", "password":"your_password"}'```

### JWT token validation
```curl -X GET http://x.x.x.x:4000/validate -H "Authorization: <token>"```
